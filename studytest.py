...
#读取excel表格
def excel_devide(filePath,dirs,aimpath):   #读取转存
...
#查找data中有动态数据的组
    ...
    # 将有动态数据的组提取出来存入新的xlsx组内
        ...
        b=dirs[dirs_count]
        print(b)
        data_new.to_excel('{add3}\{add1}\{add2}-静态数据.xlsx'.format(add1=str(b),add2=str(b),add3=str(aimpath)))
        dirs_count+=1